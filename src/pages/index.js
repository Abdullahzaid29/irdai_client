...
const { isAuthenticated } = useAuth();
  ...
  const submit = async () =>{
   let  params = {
    ...
    try {
      ...
      toast('Sucessfully loged in', { hideProgressBar: true, autoClose: 2000, type: 'success' })
      // await router.push('/home');
      // if (isAuthenticated) {
      ...
      console.log(err);
      toast('invalid credentials', { hideProgressBar: true, autoClose: 2000, type: "error" })
    }
  //   if(status){
  //     await router.push('/home');
  //    toast('Sucessfully loged in', { hideProgressBar: true, autoClose: 2000, type: 'success' })
  ...
  //     'http://localhost:5000/api/signin',{method:'POST', body: 
...
//  },[])
  ...
  const forgot_password = async () =>{
    try {
      await axios.post('http://localhost:7000/api/forgotpassword',{   username:pusername})  .then(res => {
...
<link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Material+Symbols+Outlined:opsz,wght,FILL,GRAD@48,400,0,0" />
    ...
    />
            ...
            <div className="flex justify-between">
              ...
              <a href="#" className="text-xs text-blue-600" onClick={pusername?openModal:console.log("error")}>
                Forgot Password?
              </a>
            </div>
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg">
        ...
        <p className="text-gray-600 mb-4">A Link Has Been Generated To Reset Password</p>
        {/* <input
          type="email"
          placeholder="Email"
          // value={email}
        ...
        /> */}
        <button
          onClick={()=>{console.log(forgetpasslink);}}
          // href={forgetpasslink}
          className="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600"
         
        >
          <Link href={forgetpasslink} rel="noopener noreferrer" target="_blank">Open Link
 </Link>
        </button>
        <button
          onClick={closeModal}
